...
abstract class Navigation {
  ...
  @Deprecated('don\'t use this method')
  bool pushNamed(
      {required String path, required NavigationData navigationData});

  // this method is used to push new routes on screen with data as Extra
  bool pushNamedWithExtra(
      {required String path, required NavigationData navigationData});

  bool pushNamedAndRemoveWithExtra(
      {required String path, required NavigationData navigationData});

  bool pushNamedAndRemove({required String path});

  bool pushNamedWithExtraList(
      {required String path, required List<NavigationData> navigationData});
  ...
  Future<bool> pushNamedWithExtraForResult(
      {required String path, required NavigationData navigationData});
...
class NavigationImpl implements Navigation {
  ...
  bool goNamed(String path, NavigationData navigationData) {
    AppRouter.rootNavigatorKey.currentContext!
        .goNamed(path, queryParameters: navigationData.toJson());
    return true;
  }
  ...
  bool pushNamedWithExtra(
      {required String path, required NavigationData navigationData}) {
    AppRouter.rootNavigatorKey.currentContext!
        .pushNamed(path, extra: navigationData);
    return true;
  }
  ...
  bool goNamedWithExtra(
      {required String path, required NavigationData navigationData}) {
    AppRouter.rootNavigatorKey.currentContext!
        .goNamed(path, extra: navigationData);
    return true;
  }
  ...
  @override
  Future<bool> pushNamedWithExtraForResult(
      {required String path, required NavigationData navigationData}) async {
    return await AppRouter.rootNavigatorKey.currentContext!
            .pushNamed<bool>(path, extra: navigationData) ??
        false;
  }

  @override
  Future<bool> pushForResult({required String path}) async {
    return await AppRouter.rootNavigatorKey.currentContext!.push<bool>(path) ??
        false;
  }
  ...
  bool pushNamedAndRemoveWithExtra(
      {required String path, required NavigationData navigationData}) {
    AppRouter.rootNavigatorKey.currentContext!
        .pushReplacementNamed(path, extra: navigationData);
    return true;
  }
